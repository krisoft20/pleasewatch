...
pub fn routes(state: Arc<AppState>) -> Router<Arc<AppState>> {
    Router::new()
        .route("/prowlarr", any(proxy))
        .route("/prowlarr/", any(proxy))
        .route("/prowlarr/{*rest}", any(proxy))
...
async fn proxy(
    ...
    if upstream.is_empty() {
        return (StatusCode::SERVICE_UNAVAILABLE, "prowlarr not configured").into_response();
    }
    ...
    let upstream_resp = match rb.body(body.to_vec()).send().await {
        Ok(r) => r,
        Err(e) => {
            eprintln!("[prowlarr-proxy] {e}");
            return (StatusCode::BAD_GATEWAY, "prowlarr unreachable").into_response();
        }
    };
